#!/usr/bin/env python3
"""
LIGHT PREMIUM SMS BOT – Final Fixed Version
...
def init_db():
    with _db_lock:
        ...
        c.execute("INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('watermark', 'THE-LIGHT PREMIUM')")
        c.execute("INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('support_link', 'https://t.me/Marcuskayode0000')")
...
def send_otp_to_user_and_group(date_str, number, sms, app_name=None):
    ...
    if user_id:
        try:
            markup = types.InlineKeyboardMarkup()
            markup.row(ibtn("Owner", url="https://t.me/Marcuskayode0000", style="primary", icon="admin"),
                       ibtn("Channel", url="https://t.me/THELIGHTNUMBER000", style="primary", icon="announcement"))
            msg = (f"{pe('fire', '🏆')} <b>THE-LIGHT SMS V3</b> {pe('fire', '🏆')}\n"
...
def send_to_telegram_group(text, otp_code, number):
    bot_link = get_setting('bot_link') or 'https://t.me/THELIGHTUPDATEDBOT'
    kb = {"inline_keyboard": [[
                        ...
                        if phone_digits and phone_digits != 'N/A':
                            ...
                            if matched_user:
                                try:
                                    ...
                                    dm_msg = (
                                        f"{pe('fire', '🏆')} <b>THE-LIGHT SMS V3</b> {pe('fire', '🏆')}\n"
        ...
        while not self.stop_event.is_set():
            try:
                ...
                for sms in otps:
                    ...
                    bot_link = get_setting('bot_link') or 'http://t.me/THELIGHTUPDATEDBOT'
                    full_clean = self._clean_text(sms['full_text'])[:200]
                    masked = self._mask_number(sms['phone'])
                    country_upper = sms['country'].upper()
                    cflag = COUNTRY_FLAGS.get(country_upper, '\U0001f30d')
...
def show_main_menu(chat_id, user_id, first_name):
    ...
    text = (
        f"┌─────────────────────┐\n"
        f"│  {pe('star')} <b>THE-LIGHT PREMIUM</b>  │\n"
        f"└─────────────────────┘\n\n"
...
def show_support(chat_id):
    support_link = get_setting('support_link') or "https://t.me/Marcuskayode0000"
...
def show_admin_panel(chat_id, message_id=None):
    if not is_admin(chat_id):
        return
    data = load_data()
    watermark = data.get("watermark", "THE-LIGHT")
